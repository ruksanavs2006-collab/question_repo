...
def admin():
    ...
    ''').fetchall()
    courses = connection.execute('SELECT id, name FROM courses ORDER BY name').fetchall()
    connection.close()
    return render_template('admin.html', subjects=subjects, courses=courses)
...
def add_subject():                                            
    ...
    if not course_id or not semester or not name:            
        flash("Please fill in course, semester, and subject name.")
        return redirect(url_for('admin'))                    
    ...
    existing = connection.execute(                            
        'SELECT id FROM subjects WHERE course_id = ? AND semester = ? AND name = ?',
        (course_id, semester, name)                          
    ).fetchone()                                              
                                                                
    if existing:                                              
        connection.close()                                    
        flash(f'"{name}" already exists for that course and semester.')
        return redirect(url_for('admin'))                    
                                                                
    connection.execute(                                        
        'INSERT INTO subjects (course_id, semester, name) VALUES (?, ?, ?)',
        (course_id, semester, name)                          
    )                                                          
    connection.commit()                                        
    connection.close()                                        
